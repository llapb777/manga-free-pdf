...
export const render = (mangaList, mangaResult, currentItem) => {
	...
	if (mangaResult && mangaResult.length) {
		mangaResult.forEach(item => {
			...
			const name = `${item.name.slice(0, 30).trim()}${
				item.name.length > 30 ? "..." : ""
			}`
			const hideImage = `${!item.imgLink ? "no-photo" : ""}`

			root.innerHTML += `
			<div class="item">
          <button id=${item.id} class="item__tg-link ${hideImage}" title="${item.name}" target="_blank" >${imgLink}
			</button>
          <button id=${item.id} class="item__link" title="${item.name}">${name}</button>
        </div>`
		})
